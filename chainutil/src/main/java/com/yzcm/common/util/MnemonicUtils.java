...
public class MnemonicUtils {
    ...
    public static String generateMnemonic(byte[] initialEntropy) {
        ...
        List<String> mne = Lists.newArrayList();
    ...
    public static boolean checkMnemonic(String mnemonicStr) {
        try {
        List<String> mnemonic = Arrays.asList(mnemonicStr.split(" "));
        Map<String, String> wordIndex = Maps.newHashMap();
        for (int i=0;i<WORD_LIST.size();i++) {
            wordIndex.put(WORD_LIST.get(i),get11BitBinString(i));
        }
        StringBuffer bits = new StringBuffer();
        for (int i=0;i<mnemonic.size();i++) {
            bits.append(wordIndex.get(mnemonic.get(i)));
        }
        String bitsStr = bits.toString();
        String checkByte = bitsStr.substring(bitsStr.length()-4,bitsStr.length());
        byte[] iterations = string2bytes(bitsStr.substring(0,bitsStr.length()-4));
        byte checksum = calculateChecksum(iterations);
        byte[] checksums = string2bytes(checkByte);
    ...
    /**
     */
    public static String get11BitBinString(int number) {
